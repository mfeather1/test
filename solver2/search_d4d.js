/* Dist4: 14.27 GB in 8 parts 
   p1 = 70*2187*782*16  16 = (9 of 11 et bits = 512) / 4 configs/byte / 8 parts
   p2 = p01*2, p03 = p01*3, etc
*/
const p1 = 1915462080, p2 = 3830924160,  p3 = 5746386240, p4 = 7661848320;
const p5 = 9577310400, p6 = 11492772480, p7 = 13408234560;
  ...
  for (var i=0, mv=0; (mv=mvlist[i]) != -1; i++) {
    ...
    if (n == depth) {
      ...
      if (depth-n <= 11 && depth-n >= 9) {
        var rs = etsym>>2;
        ix = (cpsym*2187+ctsym)*100096 + epmin*128 + (rs>>2);  // MIN_EP*128 = 100096 
        tmp = (ix < p4) ?
          ((ix < p2) ? 
            ((ix < p1) ? tmp = dist401[ix]    : tmp = dist402[ix-p1]) :
            ((ix < p3) ? tmp = dist403[ix-p2] : tmp = dist404[ix-p3])) :
          ((ix < p6) ? 
            ((ix < p5) ? tmp = dist405[ix-p4] : tmp = dist406[ix-p5]) :
